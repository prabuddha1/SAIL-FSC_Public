...
#System Requirements:
----------------------------------------------
SAIL-FSC 1.0 has been tested on the following setup:
...
SAIL-FSC 1.0 currently supports flattened Verilog designs and Cadence GSC 180nm standard cell library with the following gates only:
...
**********************************************
#Training a SAIL-FSC Model From Scratch:
--------------------------------------------

Executable to use: SAIL-FSC_TrainModel 
--------------------------------------------

Sample Command:

./SAIL-FSC_TrainModel -XNodeCut 3_4_5 -XInDepthCutoff 2 -XOutDepthCutoff 2 -unityDSPath ./data/ -verbose 0 -modelOutPath ./model.npy
...
-modelOutPath: The path and name of the trained model file that SAIL-FSC will be generating. Please use ".npy" as the extension. The script will overwrite any exisiting file at this path. 

--------------------------------------------


**********************************************
#Evaluating a <netlist, Ground Truth Label> pair using a pre-trained SAIL-FSC model:
--------------------------------------------

Executable to use: SAIL-FSC_Evaluate (Run it from inside ./binary/)
--------------------------------------------

Sample Commands:

./SAIL-FSC_Evaluate -XNodeCut 3_4_5 -XInDepthCutoff 2 -XOutDepthCutoff 2 -modelPath model.npy -tempFolderPath ./data/tmp/ -verbose 0 -datasetpath_netlist ./data/netlists/0_c1355.v -datasetpath_label ./data/labels/0_c1355.label
...
-modelPath: The path to the trained SAIL-FSC model.
...
The predictions are stored in <tempFolderPath>/dataset/SAIL-FSC_results
...
2) Inside the labels file, each line indicates <Gate Name, Vulnerability Metric>
